...
#  from settings import VERBOSE, CONSUMER_DATA, PRODUCT_DATA
#  from settings import SIMULATE, NUM_PRODUCT, NUM_TYPE, NUM_FEATURE
...
if not settings.SIMULATE:
    ...
    K = settings.NUM_TYPE
    N = settings.NUM_PRODUCT
    M = settings.NUM_FEATURE
    pop = Population.from_sim(num_types=K, feature_len=M)
    ps = Product_Set.from_sim(num_prod=N, feature_len=M)
    if settings.SAVE_DATA:
        ps.save("data/prod.sim")
        pop.save("data/cons.sim")
...
class FrankWolfe(object):
    ...
    def __init__(self, exp, params, verbose=VERBOSE):
    ...
    def find_away_direction(active_set, y_vec, g):
        logger.warning(f"Searching for away direction:")
        logger.info(f"current active set:")
        for k in active_set:
            logger.debug(f"{k}: {active_set[k]}")

        best_obj = -np.Inf

        for k in active_set:
            loss = np.dot(np.array(g) - y_vec, active_set[k])
            if loss > best_obj:
                best_obj = loss
                away_dir = active_set[k]
                best_index = k

        away_direction = active_set[best_index]
        logger.info(f"direction chosen: {best_index};{away_direction}")
        logger.warning(f"Searching finished")

        return best_index, away_direction

    def isBoundaryType(self, f):
        return True if np.min(f) <= 1e-3 else False


if __name__ == "__main__":
    #  consumer_data = "consumer.csv"
